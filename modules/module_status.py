...
class ModuleStatus(commands.Cog):
    ...
    async def mod_status(self, ctx):
        embed = discord.Embed(title="Status Modules", description="Test", color=discord.Color.blue())

        modules = [f[:-3] for f in os.listdir('./modules') if f.endswith('.py') and not f.startswith('_')]
        for module in modules:
            description = self.module_descriptions.get(module, "Nothing")
            status_emoji = "🟢" if f'modules.{module}' in self.bot.extensions else "🔴"
            embed.add_field(name=f"{status_emoji} {module}", value=description, inline=False)
